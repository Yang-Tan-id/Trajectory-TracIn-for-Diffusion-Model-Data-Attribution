#!/usr/bin/env bash
#SBATCH -J 3d-pn12-postsq
#SBATCH -o 3d-pn12-postsq-%j.out
#SBATCH -e 3d-pn12-postsq-%j.err
#SBATCH -p rtx-small
#SBATCH -N 1
#SBATCH -n 2
...
NUM_PROBES=12
# Preserve the historical namespace so existing independent probes r0-r7 are
# reused. The query driver skips complete artifacts and creates only r8-r11.
QUERY_PATTERN='loss_direction_residual_rms_predicted_noise_probe4_r{probe_index}'
LOG_ROOT="${SHAPES_ROOT}/result/${EXPERIMENT_TAG}/logs/predicted_noise_probe12_final_post_square/${SLURM_JOB_ID}"
mkdir -p "${LOG_ROOT}"

echo "[phase 1/3] ensure twelve independent query-probe banks; reuse 0-7 and generate missing 8-11"
...
echo "[phase 2/3] original train features x twelve probes; emit three final reductions"
pids=()
for shard in 0 1; do
  (
...
JAX_PLATFORMS=cpu "${PYTHON_BIN}" "${SHAPES_ROOT}/script/run_traj_tracin_lds_cached.py" \
  ...
  --experiment "${EXPERIMENT_TAG}" \
  --train-seed "${TRAIN_SEED}" \
  --score-schemes predicted_noise_jvp_final_linear_mean_probe12 \
  --prediction-sign=1
...
JAX_PLATFORMS=cpu "${PYTHON_BIN}" "${SHAPES_ROOT}/script/run_traj_tracin_lds_cached.py" \
  ...
  --train-seed "${TRAIN_SEED}" \
  --score-schemes \
    predicted_noise_jvp_final_square_then_mean_probe12,predicted_noise_jvp_final_mean_then_square_probe12 \
  --prediction-sign=-1
...
echo "[done] twelve-probe scores and LDS are complete"
