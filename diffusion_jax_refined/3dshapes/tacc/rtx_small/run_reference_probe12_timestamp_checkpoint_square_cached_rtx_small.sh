#!/usr/bin/env bash
#SBATCH -J 3d-ref12-tsq
#SBATCH -o 3d-ref12-tsq-%j.out
#SBATCH -e 3d-ref12-tsq-%j.err
#SBATCH -p rtx-small
#SBATCH -N 1
#SBATCH -n 2
...
LOG_ROOT="${SHAPES_ROOT}/result/${EXPERIMENT_TAG}/logs/reference_probe12_timestamp_checkpoint_square/${SLURM_JOB_ID}"
mkdir -p "${LOG_ROOT}"

echo "[score] fixed reference: for each probe/timestamp, sum signed linear terms over checkpoints, then square"
pids=()
for shard in 0 1; do
  (
...
SCORE_SCHEME=predicted_noise_jvp_timestamp_checkpoint_sum_square_probe12_reference_timestamp_shared12
...
for sign in p1 m1; do
  "${PYTHON_BIN}" "${SHAPES_ROOT}/script/print_predicted_noise_timestamp_checkpoint_square_lds.py" \
    ...
    --reduction timestamp_checkpoint_square --prediction-sign "${sign}" \
    --namespace-suffix "${NAMESPACE_SUFFIX}"
done

echo "[done] fixed-reference 12-probe timestamp/checkpoint linear-sum square LDS"
