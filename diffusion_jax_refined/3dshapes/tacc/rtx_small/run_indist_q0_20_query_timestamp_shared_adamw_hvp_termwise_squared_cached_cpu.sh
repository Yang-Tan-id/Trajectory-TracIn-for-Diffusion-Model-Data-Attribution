#!/usr/bin/env bash
#SBATCH -J 3d-q0-20-qht2
#SBATCH -o 3d-q0-20-qht2-%j.out
#SBATCH -e 3d-q0-20-qht2-%j.err
#SBATCH -p skx-dev
#SBATCH -N 1
#SBATCH -n 1
...
residual_namespace="adamw_residual_aligned10x10_qts_probe_second_order_hvp_next_delta_termwise_squared_q0_20"
full_namespace="adamw_full_aligned10x10_qts_probe_second_order_hvp_next_delta_termwise_squared_q0_20"
...
echo '[1/3] residual AdamW second-order query-timestamp-shared termwise square'
...
echo '[2/3] full AdamW second-order query-timestamp-shared termwise square'
python script/run_traj_tracin_queries_and_scores.py \
  --execute --experiment "$experiment" --train-seed "$seed" \
  --query-file "$query_file" --query-ids "$query_ids" --gpus 0 \
  --skip-sampling --skip-query-gradient \
...
echo '[done] Q0-Q20 second-order query-timestamp-shared HVP termwise square'
