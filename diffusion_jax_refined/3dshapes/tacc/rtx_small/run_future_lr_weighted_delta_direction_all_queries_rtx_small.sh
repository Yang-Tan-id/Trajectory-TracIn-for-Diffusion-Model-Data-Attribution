#!/usr/bin/env bash
#SBATCH -J 3d-futlr10q
#SBATCH -o 3d-futlr10q-%j.out
#SBATCH -e 3d-futlr10q-%j.err
#SBATCH -p rtx-small
#SBATCH -N 1
#SBATCH -n 2
...
ORIGINAL_NAMESPACE="predicted_noise_output_future_lr_delta_original12"
FRESH_NAMESPACE="predicted_noise_output_future_lr_delta_fresh12"
OUT_DIR="${SHAPES_ROOT}/result/${EXPERIMENT_TAG}/eval/future_lr_weighted_delta_direction_all_queries/run_${SLURM_JOB_ID}"
mkdir -p "${OUT_DIR}"
...
export TRAJ_TRACIN_PROBE_ALIGNMENT_FUTURE_LR_WEIGHTED_DELTA=1

echo "[phase 1/4] LR-weighted sum of all future adjacent noise deltas; original probes | Q=${QUERY_IDS}"
python "${SHAPES_ROOT}/script/run_traj_tracin_queries_and_scores.py" \
  --execute --experiment "${EXPERIMENT_TAG}" --train-seed "${TRAIN_SEED}" \
  --epochs "${JAX_EPOCHS}" --query-ids "${QUERY_IDS}" --gpus 0,1 \
  --skip-sampling --skip-score --artifact-namespace "${ORIGINAL_NAMESPACE}" \
  --query-objective trajectory_predicted_noise_probe \
  --predicted-noise-probe-count 12 --num-snapshots 10 \
  --log-prefix futlr10q_original12

echo "[phase 2/4] LR-weighted sum of all future adjacent noise deltas; fresh probes | Q=${QUERY_IDS}"
python "${SHAPES_ROOT}/script/run_traj_tracin_queries_and_scores.py" \
  --execute --experiment "${EXPERIMENT_TAG}" --train-seed "${TRAIN_SEED}" \
  --epochs "${JAX_EPOCHS}" --query-ids "${QUERY_IDS}" --gpus 0,1 \
  --skip-sampling --skip-score --artifact-namespace "${FRESH_NAMESPACE}" \
  --query-objective trajectory_predicted_noise_probe \
  --predicted-noise-probe-count 12 --predicted-noise-probe-seed 20260915 \
  --num-snapshots 10 --log-prefix futlr10q_fresh12

unset TRAJ_TRACIN_PROBE_ALIGNMENT_ONLY
unset TRAJ_TRACIN_PROBE_ALIGNMENT_NEXT_CHECKPOINT
unset TRAJ_TRACIN_PROBE_ALIGNMENT_FUTURE_LR_WEIGHTED_DELTA

echo "[phase 3/4] select the nearest of 24 probes to the LR-weighted future direction"
pids=()
for shard in 0 1; do
  (
    CUDA_VISIBLE_DEVICES="${shard}" JAX_NUM_DEVICES=1 JAX_PLATFORMS=cuda \
      "${PYTHON_BIN}" "${ANALYZER}" analyze-shard \
        ...
        --checkpoint-direction next --include-future-lr-weighted-delta
...
echo "[phase 4/4] merge and evaluate all 10 queries"
...
echo "[done] LR-weighted future predicted-noise delta LDS complete"
