#!/usr/bin/env bash
#SBATCH -J 3d-dmc24
#SBATCH -o 3d-dmc24-%j.out
#SBATCH -e 3d-dmc24-%j.err
#SBATCH -p rtx-small
#SBATCH -N 1
#SBATCH -n 2
...
OUT_DIR="${SHAPES_ROOT}/result/${EXPERIMENT_TAG}/eval/delta_noise_mc24/run_${SLURM_JOB_ID}"
mkdir -p "${OUT_DIR}"

echo "[phase 1/2] cached 24-probe Monte Carlo estimate of P J^T delta-epsilon"
pids=()
for shard in 0 1; do
  (
    CUDA_VISIBLE_DEVICES="${shard}" JAX_NUM_DEVICES=1 JAX_PLATFORMS=cuda \
      "${PYTHON_BIN}" "${ANALYZER}" analyze-shard \
        ...
        --checkpoint-direction next --include-delta-mc24
...
echo "[done] sign-invariant 24-probe delta pullback LDS complete"
