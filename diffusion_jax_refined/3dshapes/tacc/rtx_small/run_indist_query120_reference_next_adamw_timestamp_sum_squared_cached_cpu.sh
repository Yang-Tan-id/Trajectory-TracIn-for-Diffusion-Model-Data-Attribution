#!/usr/bin/env bash
#SBATCH -J 3d-id120-tsq
#SBATCH -o 3d-id120-tsq-%j.out
#SBATCH -e 3d-id120-tsq-%j.err
#SBATCH -p spr
#SBATCH -N 1
#SBATCH -n 1
...
count="$(find "$shapes/result/$experiment/sample_ddim_eta0_1000" -type f \
  -path "*/seed_*_query_gradient_${query_namespace}/traj_tracin/query_gradient_artifact.npz" \
  | wc -l | tr -d ' ')"
[[ "$count" == 120 ]] || { echo "Expected 120 cached reference query artifacts, found $count" >&2; exit 1; }

cd "$shapes"
echo '[1/3] residual: sum AdamW dots over checkpoints within timestamp, then square'
...
echo '[2/3] full AdamW: add optimizer history, sum checkpoints within timestamp, then square'
python script/run_traj_tracin_queries_and_scores.py \
  --execute --experiment "$experiment" --train-seed "$seed" \
  --query-file "$query_file" --query-ids "$query_ids" --gpus 0 \
  --skip-sampling --skip-query-gradient \
...
echo '[3/3] cached LDS for residual/full and four normalization variants'
...
echo '[done] Q0-Q119 reference AdamW timestamp-sum-then-square'
