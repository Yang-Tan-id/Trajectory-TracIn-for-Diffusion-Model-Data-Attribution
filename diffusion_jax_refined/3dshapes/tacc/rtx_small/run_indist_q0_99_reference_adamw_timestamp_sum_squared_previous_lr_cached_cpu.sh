#!/usr/bin/env bash
#SBATCH -J 3d-q0-99-atsq
#SBATCH -o 3d-q0-99-atsq-%j.out
#SBATCH -e 3d-q0-99-atsq-%j.err
#SBATCH -p skx-dev
#SBATCH -N 1
#SBATCH -n 1
...
residual_namespace="adamw_residual_aligned10x10_timestamp_sum_squared_previous_lr_ref100q"
full_namespace="adamw_full_aligned10x10_timestamp_sum_squared_previous_lr_ref100q"
...
query_list="${SLURM_TMPDIR:-/tmp}/reference_q0_99_adamw_${SLURM_JOB_ID:-$$}.txt"
...
cd "$shapes"
echo '[1/3] AdamW residual: sum linear dots across checkpoints per timestamp, then square'
...
echo '[2/3] full AdamW: add optimizer history before timestamp-wise contraction'
python script/run_traj_tracin_queries_and_scores.py \
  --execute --experiment "$experiment" --train-seed "$seed" \
  --query-file "$query_file" --query-ids "$query_ids" --gpus 0 \
  --skip-sampling --skip-query-gradient \
...
echo '[3/3] 3200 cached LDS evaluations for residual and full AdamW'
...
echo '[done] Q0-Q99 AdamW-aware timestamp-wise-square/previous-LR'
