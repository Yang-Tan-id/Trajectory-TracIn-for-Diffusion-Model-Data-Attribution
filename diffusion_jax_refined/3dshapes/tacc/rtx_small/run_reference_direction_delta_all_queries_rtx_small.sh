#!/usr/bin/env bash
#SBATCH -J 3d-refunit10q
#SBATCH -o 3d-refunit10q-%j.out
#SBATCH -e 3d-refunit10q-%j.err
#SBATCH -p rtx-small
#SBATCH -N 1
#SBATCH -n 2
...
ORIGINAL_NAMESPACE="predicted_noise_output_reference_direction_delta_original12"
FRESH_NAMESPACE="predicted_noise_output_reference_direction_delta_fresh12"
OUT_DIR="${SHAPES_ROOT}/result/${EXPERIMENT_TAG}/eval/reference_direction_delta_all_queries/run_${SLURM_JOB_ID}"
mkdir -p "${OUT_DIR}"
...
echo "[phase 1/4] unit(reference noise)-unit(current noise); original probes | Q=${QUERY_IDS}"
python "${SHAPES_ROOT}/script/run_traj_tracin_queries_and_scores.py" \
  ...
  --log-prefix refunit10q_original12

echo "[phase 2/4] unit(reference noise)-unit(current noise); fresh probes | Q=${QUERY_IDS}"
python "${SHAPES_ROOT}/script/run_traj_tracin_queries_and_scores.py" \
  --execute --experiment "${EXPERIMENT_TAG}" --train-seed "${TRAIN_SEED}" \
  --epochs "${JAX_EPOCHS}" --query-ids "${QUERY_IDS}" --gpus 0,1 \
  ...
  --num-snapshots 10 --log-prefix refunit10q_fresh12

unset TRAJ_TRACIN_PROBE_ALIGNMENT_ONLY
unset TRAJ_TRACIN_PROBE_ALIGNMENT_NEXT_CHECKPOINT

echo "[phase 3/4] select the nearest of 24 probes to the normalized direction change"
pids=()
for shard in 0 1; do
  (
    CUDA_VISIBLE_DEVICES="${shard}" JAX_NUM_DEVICES=1 JAX_PLATFORMS=cuda \
      "${PYTHON_BIN}" "${ANALYZER}" analyze-shard \
        ...
        --checkpoint-direction next --include-reference-direction-delta
...
echo "[done] normalized current-to-final predicted-noise direction LDS complete"
