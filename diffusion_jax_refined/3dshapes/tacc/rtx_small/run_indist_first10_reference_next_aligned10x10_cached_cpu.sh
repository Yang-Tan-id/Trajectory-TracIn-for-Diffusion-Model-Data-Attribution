#!/usr/bin/env bash
#SBATCH -J 3d-id10-a10cpu
#SBATCH -o 3d-id10-a10cpu-%j.out
#SBATCH -e 3d-id10-a10cpu-%j.err
#SBATCH -p spr
#SBATCH -N 1
#SBATCH -n 1
...
cd "$shapes"
echo '[1/3] residual aligned10x10: crop cached reference 100t queries to 10 timestamps'
...
echo '[2/3] full AdamW aligned10x10 using stored optimizer history'
python script/run_traj_tracin_queries_and_scores.py \
  --execute --experiment "$experiment" --train-seed "$seed" \
  --query-file "$query_file" --query-ids "$query_ids" --gpus 0 \
  --skip-sampling --skip-query-gradient \
...
echo '[done] ID first10 reference-next aligned10x10'
