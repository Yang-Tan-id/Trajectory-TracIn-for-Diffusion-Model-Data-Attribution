#!/usr/bin/env bash
#SBATCH -J 3d-delta10q
#SBATCH -o 3d-delta10q-%j.out
#SBATCH -e 3d-delta10q-%j.err
#SBATCH -p rtx-small
#SBATCH -N 1
#SBATCH -n 2
...
OUT_DIR="${SHAPES_ROOT}/result/${EXPERIMENT_TAG}/eval/delta_noise_direction_all_queries/run_${SLURM_JOB_ID}"
mkdir -p "${OUT_DIR}"
...
echo "[phase 1/4] ensure original-bank output alignments for Q=${QUERY_IDS}"
python "${SHAPES_ROOT}/script/run_traj_tracin_queries_and_scores.py" \
  ...
  --log-prefix delta10q_original12

echo "[phase 2/4] ensure fresh-bank output alignments for Q=${QUERY_IDS}"
python "${SHAPES_ROOT}/script/run_traj_tracin_queries_and_scores.py" \
  --execute --experiment "${EXPERIMENT_TAG}" --train-seed "${TRAIN_SEED}" \
  --epochs "${JAX_EPOCHS}" --query-ids "${QUERY_IDS}" --gpus 0,1 \
  --skip-sampling --skip-score \
  ...
  --num-snapshots 10 --log-prefix delta10q_fresh12

unset TRAJ_TRACIN_PROBE_ALIGNMENT_ONLY
unset TRAJ_TRACIN_PROBE_ALIGNMENT_NEXT_CHECKPOINT

echo "[phase 3/4] score Q=${QUERY_IDS} using current/next/delta selected probe directions"
pids=()
for shard in 0 1; do
  (
...
echo "[done] all-query delta predicted-noise direction LDS complete"
