...
ids="$(seq -s, 0 99)"
train_ns="factorized_mc4_reference100x1"
query_ns="original100q_100x1"
score_ns="factorized_mc4_original100q_100x1"
python "$shapes/script/build_queries.py" --num-queries 100 --output "$query_file"

echo '[1/2] store Q0-Q99 DAS original endpoint-renoise query gradients and sweep 16 lambdas on two GPUs'
python "$shapes/script/run_das_queries_and_scores.py" \
  ...
  --artifact-namespace "$query_ns" --train-artifact-namespace "$train_ns" \
  --score-output-namespace "$score_ns" --query-input-mode endpoint_renoise \
  --num-mc-noise 1

echo '[2/2] cached DAS LDS: 100 queries x 16 lambdas x four targets'
JAX_PLATFORMS=cpu python "$shapes/script/run_das_lds_cached.py" \
  --execute --experiment "$experiment" --train-seed "$seed" \
  --query-file "$query_file" --query-ids "$ids" \
  --artifact-namespace "$score_ns" --prediction-sign -1
echo '[done] DAS factorized-MC4 original Q0-Q99'
