#!/usr/bin/env bash
#SBATCH -J 3d-adam-hist
#SBATCH -o 3d-adam-hist-%j.out
#SBATCH -e 3d-adam-hist-%j.err
#SBATCH -p rtx-small
#SBATCH -N 1
#SBATCH -n 1
...
while [[ "${candidate}" != "/" && ! -f "${candidate}/diffusion_jax_refined/3dshapes/script/analyze_optimizer_history_update.py" ]]; do
...
NAMESPACE="${NAMESPACE:-optimizer_history_update_sanity_reference}"
RESULT_ROOT="${SHAPES_ROOT}/result/${EXPERIMENT_TAG}"
OUTDIR="${RESULT_ROOT}/eval/optimizer_history_update_sanity_reference/run_${SLURM_JOB_ID}"
