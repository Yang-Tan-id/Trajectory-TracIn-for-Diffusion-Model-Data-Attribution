#!/usr/bin/env bash
#SBATCH -J 3d-dmc24sq
#SBATCH -o 3d-dmc24sq-%j.out
#SBATCH -e 3d-dmc24sq-%j.err
#SBATCH -p rtx-small
#SBATCH -N 1
#SBATCH -n 2
...
OUT_DIR="${SHAPES_ROOT}/result/${EXPERIMENT_TAG}/eval/delta_noise_mc24_product_square/run_${SLURM_JOB_ID}"
mkdir -p "${OUT_DIR}"

echo "[phase 1/2] cached MC-JTP-24 termwise product-square; four L2 variants"
pids=()
for shard in 0 1; do
  (
    CUDA_VISIBLE_DEVICES="${shard}" JAX_NUM_DEVICES=1 JAX_PLATFORMS=cuda \
      "${PYTHON_BIN}" "${ANALYZER}" analyze-shard \
        ...
        --checkpoint-direction next --include-delta-mc24-product-square
...
echo "[phase 2/2] merge and evaluate all queries"
...
echo "[done] MC-JTP-24 product-square LDS complete"
