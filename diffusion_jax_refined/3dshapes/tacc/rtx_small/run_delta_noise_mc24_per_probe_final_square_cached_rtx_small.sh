#!/usr/bin/env bash
#SBATCH -J 3d-dmc24pfsq
#SBATCH -o 3d-dmc24pfsq-%j.out
#SBATCH -e 3d-dmc24pfsq-%j.err
#SBATCH -p rtx-small
#SBATCH -N 1
#SBATCH -n 2
...
ORIGINAL_NAMESPACE="predicted_noise_output_next_original12"
FRESH_NAMESPACE="predicted_noise_output_next_fresh12"
OUT_DIR="${SHAPES_ROOT}/result/${EXPERIMENT_TAG}/eval/delta_noise_mc24_per_probe_final_square/run_${SLURM_JOB_ID}"
mkdir -p "${OUT_DIR}"

echo "[phase 1/2] complete each delta-weighted probe score; square final scores; mean 24 probes"
pids=()
for shard in 0 1; do
  (
...
echo "[phase 2/2] merge complete trajectories, square each probe, and evaluate"
...
echo "[done] MC-JTP-24 per-probe final-square LDS complete"
