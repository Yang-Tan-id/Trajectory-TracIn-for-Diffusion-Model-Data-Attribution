#!/usr/bin/env bash
#SBATCH -J 3d-pn8-tsq
#SBATCH -o 3d-pn8-tsq-%j.out
#SBATCH -e 3d-pn8-tsq-%j.err
#SBATCH -p rtx-small
#SBATCH -N 1
#SBATCH -n 2
...
LOG_ROOT="${SHAPES_ROOT}/result/${EXPERIMENT_TAG}/logs/predicted_noise_probe8_checkpoint_timestamp_square/${SLURM_JOB_ID}"
mkdir -p "${LOG_ROOT}"

echo "[phase 1/2] per checkpoint/probe: mean ten timestamps, square, then mean eight probes"
pids=()
for shard in 0 1; do
  (
...
JAX_PLATFORMS=cpu "${PYTHON_BIN}" "${SHAPES_ROOT}/script/run_traj_tracin_lds_cached.py" \
  ...
  --score-schemes predicted_noise_jvp_checkpoint_timestamp_sum_square_probe8 \
  --prediction-sign=-1
...
echo "[done] eight-probe checkpoint timestamp-mean square LDS complete"
