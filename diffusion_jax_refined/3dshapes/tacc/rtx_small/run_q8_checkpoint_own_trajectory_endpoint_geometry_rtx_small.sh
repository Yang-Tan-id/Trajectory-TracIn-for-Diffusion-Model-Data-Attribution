#!/usr/bin/env bash
#SBATCH -J 3d-q8-ownend
#SBATCH -o 3d-q8-ownend-%j.out
#SBATCH -e 3d-q8-ownend-%j.err
#SBATCH -p rtx-small
#SBATCH -N 1
#SBATCH -n 1
...
while [[ "${candidate}" != "/" && ! -f "${candidate}/diffusion_jax_refined/3dshapes/script/analyze_q8_checkpoint_own_trajectory_endpoints.py" ]]; do
...
NAMESPACE="loss_direction_original_f_checkpoint_own_trajectory_endpoints"
OUT_DIR="${SHAPES_ROOT}/result/${EXPERIMENT_TAG}/eval/q8_checkpoint_own_trajectory_endpoint_geometry/run_${SLURM_JOB_ID}"
...
echo "[phase 1/2] regenerate Q8 own trajectories and persist endpoints"
python "${SHAPES_ROOT}/script/run_traj_tracin_queries_and_scores.py" \
  ...
  --num-snapshots 10 --log-prefix q8_ownend

unset TRAJ_QUERY_USE_CHECKPOINT_OWN_TRAJECTORY

echo "[phase 2/2] compare every checkpoint endpoint with reference endpoint"
JAX_PLATFORMS=cpu python \
  "${SHAPES_ROOT}/script/analyze_q8_checkpoint_own_trajectory_endpoints.py" \
...
echo "[done] Q8 own-trajectory endpoint geometry: ${OUT_DIR}"
