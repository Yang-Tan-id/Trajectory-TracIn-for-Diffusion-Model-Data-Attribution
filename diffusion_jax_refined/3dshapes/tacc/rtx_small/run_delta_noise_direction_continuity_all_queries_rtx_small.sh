#!/usr/bin/env bash
#SBATCH -J 3d-dcont10q
#SBATCH -o 3d-dcont10q-%j.out
#SBATCH -e 3d-dcont10q-%j.err
#SBATCH -p rtx-small
#SBATCH -N 1
#SBATCH -n 2
...
ORIGINAL_NAMESPACE="predicted_noise_output_delta_continuity_original12"
FRESH_NAMESPACE="predicted_noise_output_delta_continuity_fresh12"
OUT_DIR="${SHAPES_ROOT}/result/${EXPERIMENT_TAG}/eval/delta_noise_direction_continuity_all_queries/run_${SLURM_JOB_ID}"
mkdir -p "${OUT_DIR}"
...
echo "[phase 1/4] adjacent-delta continuity signs; original probes | Q=${QUERY_IDS}"
python "${SHAPES_ROOT}/script/run_traj_tracin_queries_and_scores.py" \
  ...
  --log-prefix dcont10q_original12

echo "[phase 2/4] adjacent-delta continuity signs; fresh probes | Q=${QUERY_IDS}"
python "${SHAPES_ROOT}/script/run_traj_tracin_queries_and_scores.py" \
  --execute --experiment "${EXPERIMENT_TAG}" --train-seed "${TRAIN_SEED}" \
  --epochs "${JAX_EPOCHS}" --query-ids "${QUERY_IDS}" --gpus 0,1 \
  ...
  --num-snapshots 10 --log-prefix dcont10q_fresh12
...
echo "[phase 3/4] next-delta probe selection; flip terms whose adjacent deltas oppose"
pids=()
for shard in 0 1; do
  (
    CUDA_VISIBLE_DEVICES="${shard}" JAX_NUM_DEVICES=1 JAX_PLATFORMS=cuda \
      "${PYTHON_BIN}" "${ANALYZER}" analyze-shard \
        ...
        --checkpoint-direction next --include-delta-continuity
...
echo "[done] next-delta continuity-signed LDS complete"
