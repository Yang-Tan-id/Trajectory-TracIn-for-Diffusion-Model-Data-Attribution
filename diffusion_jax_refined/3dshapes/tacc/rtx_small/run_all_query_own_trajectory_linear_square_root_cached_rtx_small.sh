#!/usr/bin/env bash
#SBATCH -J 3d-own-lsr
#SBATCH -o 3d-own-lsr-%j.out
#SBATCH -e 3d-own-lsr-%j.err
#SBATCH -p rtx-small
#SBATCH -N 1
#SBATCH -n 1
...
OUT_DIR="${SHAPES_ROOT}/result/${EXPERIMENT_TAG}/eval/all_query_own_trajectory_linear_square_root/run_${SLURM_JOB_ID}"
...
CUDA_VISIBLE_DEVICES=0 JAX_NUM_DEVICES=1 JAX_PLATFORMS=cuda python \
  ...
  --timestep-weighting uniform --repeats 20 --random-seed 20260916 \
  --out-dir "${OUT_DIR}"

echo "[done] old own-trajectory predicted-noise-difference linear/square/root: ${OUT_DIR}"
