#!/usr/bin/env bash
#SBATCH -J 3d-pnoise-r4
#SBATCH -o 3d-pnoise-r4-%j.out
#SBATCH -e 3d-pnoise-r4-%j.err
#SBATCH -p rtx-small
#SBATCH -N 1
#SBATCH -n 2
...
NUM_PROBES=4
LOG_ROOT="${SHAPES_ROOT}/result/${EXPERIMENT_TAG}/logs/predicted_noise_jvp_l2_squared_probe4_rtx/${SLURM_JOB_ID}"
mkdir -p "${LOG_ROOT}"
...
echo "3D Shapes predicted-noise JVP L2-squared, four query probes per term"
echo "reuse=train TrajTracIn 50 checkpoints x 10 timestamps x 10-MC mean loss"
echo "estimator=mean_r square(train_gradient dot query_probe_gradient_r)"
echo "outputs=raw, query-L2, train-L2, and query+train-L2"
echo "logs=${LOG_ROOT}"
nvidia-smi

echo "[phase 1/4] four transient query-probe banks"
for probe_index in 0 1 2 3; do
  "${PYTHON_BIN}" "${QUERY_DRIVER}" \
    ...
    --skip-sampling \
    --skip-score \
    --artifact-namespace "predicted_noise_jvp_l2_squared_probe4_r${probe_index}" \
...
wait_all "four-probe score" "${score_pids[@]}"
...
echo "[phase 4/4] cached LDS for all four normalization variants"
JAX_PLATFORMS=cpu "${PYTHON_BIN}" "${SHAPES_ROOT}/script/run_traj_tracin_lds_cached.py" \
  --execute \
  --experiment "${EXPERIMENT_TAG}" \
  --train-seed "${TRAIN_SEED}" \
  --score-schemes predicted_noise_jvp_l2_squared_probe4

echo "[done] score namespace=traj_tracin_predicted_noise_jvp_l2_squared_probe4"
