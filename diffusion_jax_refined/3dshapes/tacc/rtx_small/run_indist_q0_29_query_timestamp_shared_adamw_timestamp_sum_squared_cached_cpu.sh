#!/usr/bin/env bash
#SBATCH -J 3d-q0-29-qtsq
#SBATCH -o 3d-q0-29-qtsq-%j.out
#SBATCH -e 3d-q0-29-qtsq-%j.err
#SBATCH -p skx-dev
#SBATCH -N 1
#SBATCH -n 1
...
residual_namespace="adamw_residual_aligned10x10_query_timestamp_shared_timestamp_sum_squared"
full_namespace="adamw_full_aligned10x10_query_timestamp_shared_timestamp_sum_squared"
...
cd "$shapes"
echo '[1/3] residual AdamW with query-specific, checkpoint-shared v(q,t)'
...
echo '[2/3] full AdamW with optimizer history'
python script/run_traj_tracin_queries_and_scores.py \
  --execute --experiment "$experiment" --train-seed "$seed" \
  --query-file "$query_file" --query-ids "$query_ids" --gpus 0 \
  --skip-sampling --skip-query-gradient \
...
echo '[done] Q0-Q29 query-timestamp-shared AdamW timestamp-sum-square'
