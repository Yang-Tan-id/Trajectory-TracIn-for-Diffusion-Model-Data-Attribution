#!/usr/bin/env bash
#SBATCH -J 3d-id120-a10
#SBATCH -o 3d-id120-a10-%j.out
#SBATCH -e 3d-id120-a10-%j.err
#SBATCH -p spr
#SBATCH -N 1
#SBATCH -n 1
...
cd "$shapes"
echo '[1/3] Q0-Q119 AdamW residual aligned10x10; crop cached reference 100t queries'
...
echo '[2/3] Q0-Q119 full AdamW aligned10x10; add stored optimizer history'
python script/run_traj_tracin_queries_and_scores.py \
  --execute --experiment "$experiment" --train-seed "$seed" \
  --query-file "$query_file" --query-ids "$query_ids" --gpus 0 \
  --skip-sampling --skip-query-gradient \
...
echo '[3/3] cached LDS: 120 queries x residual/full x four normalizations x four targets'
...
echo '[done] Q0-Q119 reference-next AdamW aligned10x10'
