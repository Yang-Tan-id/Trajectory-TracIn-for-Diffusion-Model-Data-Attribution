#!/usr/bin/env bash
#SBATCH -J 3d-pn12-rms
#SBATCH -o 3d-pn12-rms-%j.out
#SBATCH -e 3d-pn12-rms-%j.err
#SBATCH -p rtx-small
#SBATCH -N 1
#SBATCH -n 2
...
LOG_ROOT="${SHAPES_ROOT}/result/${EXPERIMENT_TAG}/logs/predicted_noise_probe12_termwise_rms/${SLURM_JOB_ID}"
mkdir -p "${LOG_ROOT}"

echo "[phase 1/2] twelve probes: RMS across probes within every checkpoint/timestamp"
pids=()
for shard in 0 1; do
  (
...
JAX_PLATFORMS=cpu "${PYTHON_BIN}" "${SHAPES_ROOT}/script/run_traj_tracin_lds_cached.py" \
  ...
  --score-schemes predicted_noise_jvp_rms_probe12 --prediction-sign=-1
...
"${PYTHON_BIN}" "${SHAPES_ROOT}/script/print_predicted_noise_timestamp_checkpoint_square_lds.py" \
  ...
  --reduction rms --prediction-sign m1

echo "[done] twelve-probe termwise RMS scores and LDS complete"
