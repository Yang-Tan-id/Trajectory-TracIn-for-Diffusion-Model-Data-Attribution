#!/usr/bin/env bash
#SBATCH -J 3d-q8-owntraj
#SBATCH -o 3d-q8-owntraj-%j.out
#SBATCH -e 3d-q8-owntraj-%j.err
#SBATCH -p rtx-small
#SBATCH -N 1
#SBATCH -n 1
...
OUT_DIR="${SHAPES_ROOT}/result/${EXPERIMENT_TAG}/eval/q8_original_f_checkpoint_own_trajectory_crossfit/run_${SLURM_JOB_ID}"

echo "[phase 1/2] Q8 original-f query gradients on each checkpoint's own trajectory"
python "${SHAPES_ROOT}/script/run_traj_tracin_queries_and_scores.py" \
  ...
  --num-snapshots 10 --log-prefix q8_owntraj

unset TRAJ_QUERY_USE_CHECKPOINT_OWN_TRAJECTORY

echo "[phase 2/2] build checkpoint components and crossfit signs"
CUDA_VISIBLE_DEVICES=0 JAX_NUM_DEVICES=1 JAX_PLATFORMS=cuda python \
  ...
  --query-ids 8 --repeats 20 --random-seed 20260916 \
  --original-query-namespace "${NAMESPACE}" --out-dir "${OUT_DIR}"

echo "[done] Q8 checkpoint-own-trajectory crossfit: ${OUT_DIR}"
