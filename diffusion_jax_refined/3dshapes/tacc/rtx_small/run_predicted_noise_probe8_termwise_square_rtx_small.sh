#!/usr/bin/env bash
#SBATCH -J 3d-pn8-termsq
#SBATCH -o 3d-pn8-termsq-%j.out
#SBATCH -e 3d-pn8-termsq-%j.err
#SBATCH -p rtx-small
#SBATCH -N 1
#SBATCH -n 2
...
NUM_PROBES=8
# Historical namespace intentionally reuses the eight independent probe banks 0-7.
QUERY_PATTERN='loss_direction_residual_rms_predicted_noise_probe4_r{probe_index}'
LOG_ROOT="${SHAPES_ROOT}/result/${EXPERIMENT_TAG}/logs/predicted_noise_probe8_termwise_square/${SLURM_JOB_ID}"
mkdir -p "${LOG_ROOT}"

echo "[phase 1/2] eight probes: square each gradient-probe product before averaging"
pids=()
for shard in 0 1; do
  (
...
JAX_PLATFORMS=cpu "${PYTHON_BIN}" "${SHAPES_ROOT}/script/run_traj_tracin_lds_cached.py" \
  ...
  --experiment "${EXPERIMENT_TAG}" \
  --train-seed "${TRAIN_SEED}" \
  --score-schemes predicted_noise_jvp_l2_squared_probe8 \
  --prediction-sign=-1
...
echo "[done] eight-probe termwise-square scores and LDS complete"
