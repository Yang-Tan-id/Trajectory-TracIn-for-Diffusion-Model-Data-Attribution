#!/usr/bin/env bash
#SBATCH -J 3d-adam-ref1k
#SBATCH -o 3d-adam-ref1k-%j.out
#SBATCH -e 3d-adam-ref1k-%j.err
#SBATCH -p rtx-small
#SBATCH -N 1
#SBATCH -n 2
...
namespace="loss_direction_original_f_reference_trajectory_1000t"
run_root="$shapes/result/$EXPERIMENT_TAG/eval/adamw_four_event_original_f_reference_next_1000t_constant_lr/run_${SLURM_JOB_ID}"
mkdir -p "$run_root/linear" "$run_root/squared"
cd "$shapes"

echo "[phase 1/2] fixed-reference raw next-checkpoint delta: 10 queries x all 1000 timestamps"
python script/run_traj_tracin_queries_and_scores.py --execute \
  ...
  --num-snapshots 1000 --log-prefix adam_ref1000
...
echo "[done] reference full-1000 raw-next scores: $run_root"
