#!/usr/bin/env bash
#SBATCH -J 3d-traj-s100t
#SBATCH -o 3d-traj-s100t-%j.out
#SBATCH -e 3d-traj-s100t-%j.err
#SBATCH -p rtx-small
#SBATCH -N 1
#SBATCH -n 2
...
export TRAJ_TRACIN_TRAIN_TIMESTAMP_CHUNK_SIZE="${TRAJ_TRACIN_TRAIN_TIMESTAMP_CHUNK_SIZE:-10}"
...
ARTIFACT_DIR="${SHAPES_ROOT}/result/${EXPERIMENT_TAG}/model/prompted_solo/seed_${TRAIN_SEED}_train_gradient/traj_tracin_checkpoint_shared_100x1"
ARTIFACT="${ARTIFACT_DIR}/train_datapoint_gradient_artifact.npz"
PART_DIR="${ARTIFACT}.parts"
LOG_DIR="${SHAPES_ROOT}/result/${EXPERIMENT_TAG}/logs/traj_tracin_checkpoint_shared_100x1_train"
...
if [[ -f "${ARTIFACT}" ]]; then
  echo "Complete checkpoint-shared 100x1 train artifact already exists: ${ARTIFACT}"
  exit 0
fi

echo "3D Shapes checkpoint-shared Traj TracIn train gradient"
echo "per checkpoint: 100 timestamps x 1 MC -> one averaged train gradient"
echo "stored train terms=50; timestamp_chunk_size=${TRAJ_TRACIN_TRAIN_TIMESTAMP_CHUNK_SIZE}"
...
if (( failed != 0 )); then
  echo "At least one checkpoint-shared shard failed; inspect ${LOG_DIR}/gpu_*.log" >&2
  exit 1
fi
...
[[ -f "${ARTIFACT}" ]] || {
  echo "Merged checkpoint-shared artifact was not created: ${ARTIFACT}" >&2
  exit 1
}
echo "[done] merged checkpoint-shared artifact=${ARTIFACT}"
