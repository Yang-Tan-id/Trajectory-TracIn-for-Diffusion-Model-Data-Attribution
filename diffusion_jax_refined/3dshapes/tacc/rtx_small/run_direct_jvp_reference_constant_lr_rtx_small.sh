#!/usr/bin/env bash
#SBATCH -J 3d-djvp-ref
#SBATCH -o 3d-djvp-ref-%j.out
#SBATCH -e 3d-djvp-ref-%j.err
#SBATCH -p rtx-small
#SBATCH -N 1
#SBATCH -n 2
...
out="$shapes/result/$exp/direct_jvp/reference_constant_lr"
echo "[definition] direct JVP; reference trajectory; constant checkpoint weights; projection=4096; resumable by checkpoint"
pids=()
for shard in 0 1; do
 (CUDA_VISIBLE_DEVICES="$shard" JAX_NUM_DEVICES=1 JAX_PLATFORMS=cuda python "$driver" score-shard --experiment "$exp" --train-seed "$seed" --trajectory reference --out-dir "$out" --shard-id "$shard" --num-shards 2) >"direct-jvp-reference-gpu-${shard}-${SLURM_JOB_ID}.log" 2>&1 & pids+=("$!")
done
failed=0; for pid in "${pids[@]}"; do wait "$pid" || failed=1; done; (( failed == 0 )) || exit 1
schemes="$(JAX_PLATFORMS=cpu python "$driver" merge --experiment "$exp" --train-seed "$seed" --trajectory reference --out-dir "$out" --num-shards 2 | tail -n 1)"
...
echo "[done] direct-JVP reference constant-LR eight scores"
