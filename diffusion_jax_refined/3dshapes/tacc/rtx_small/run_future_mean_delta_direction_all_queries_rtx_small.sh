#!/usr/bin/env bash
#SBATCH -J 3d-futmean10q
#SBATCH -o 3d-futmean10q-%j.out
#SBATCH -e 3d-futmean10q-%j.err
#SBATCH -p rtx-small
#SBATCH -N 1
#SBATCH -n 2
...
ORIGINAL_NAMESPACE="predicted_noise_output_future_mean_delta_original12"
FRESH_NAMESPACE="predicted_noise_output_future_mean_delta_fresh12"
OUT_DIR="${SHAPES_ROOT}/result/${EXPERIMENT_TAG}/eval/future_mean_delta_direction_all_queries/run_${SLURM_JOB_ID}"
mkdir -p "${OUT_DIR}"
...
echo "[phase 1/4] all-future predicted-noise mean direction; original probes | Q=${QUERY_IDS}"
python "${SHAPES_ROOT}/script/run_traj_tracin_queries_and_scores.py" \
  ...
  --log-prefix futmean10q_original12

echo "[phase 2/4] all-future predicted-noise mean direction; fresh probes | Q=${QUERY_IDS}"
python "${SHAPES_ROOT}/script/run_traj_tracin_queries_and_scores.py" \
  --execute --experiment "${EXPERIMENT_TAG}" --train-seed "${TRAIN_SEED}" \
  --epochs "${JAX_EPOCHS}" --query-ids "${QUERY_IDS}" --gpus 0,1 \
  ...
  --num-snapshots 10 --log-prefix futmean10q_fresh12
...
echo "[phase 3/4] select probe nearest to mean_{k>c}(eps_k-eps_c)"
pids=()
for shard in 0 1; do
  (
    CUDA_VISIBLE_DEVICES="${shard}" JAX_NUM_DEVICES=1 JAX_PLATFORMS=cuda \
      "${PYTHON_BIN}" "${ANALYZER}" analyze-shard \
        ...
        --checkpoint-direction next --include-future-mean-delta
...
echo "[done] all-future mean predicted-noise Delta LDS complete"
