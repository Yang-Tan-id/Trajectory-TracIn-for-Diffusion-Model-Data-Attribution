#!/usr/bin/env bash
#SBATCH -J 3d-q0-99-ats2
#SBATCH -o 3d-q0-99-ats2-%j.out
#SBATCH -e 3d-q0-99-ats2-%j.err
#SBATCH -p skx-dev
#SBATCH -N 1
#SBATCH -n 1
...
residual_namespace="adamw_residual_aligned10x10_termwise_squared_previous_lr_ref100q"
full_namespace="adamw_full_aligned10x10_termwise_squared_previous_lr_ref100q"
...
query_list="${SLURM_TMPDIR:-/tmp}/reference_q0_99_termwise_${SLURM_JOB_ID:-$$}.txt"
...
cd "$shapes"
echo '[1/3] residual: square every aligned (checkpoint,timestamp) dot before summing'
...
echo '[2/3] full AdamW: add optimizer history, then termwise square and sum'
python script/run_traj_tracin_queries_and_scores.py \
  --execute --experiment "$experiment" --train-seed "$seed" \
  --query-file "$query_file" --query-ids "$query_ids" --gpus 0 \
  --skip-sampling --skip-query-gradient \
...
echo '[done] Q0-Q99 AdamW-aware aligned10x10 termwise-square/previous-LR'
