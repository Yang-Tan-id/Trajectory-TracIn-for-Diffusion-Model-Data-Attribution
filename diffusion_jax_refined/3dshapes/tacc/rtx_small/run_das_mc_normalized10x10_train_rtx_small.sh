#!/usr/bin/env bash
#SBATCH -J 3d-das-n10t
#SBATCH -o 3d-das-n10t-%j.out
#SBATCH -e 3d-das-n10t-%j.err
#SBATCH -p rtx-small
#SBATCH -N 1
#SBATCH -n 2
...
while [[ "${candidate}" != "/" ]]; do
  [[ -f "${candidate}/diffusion_jax_refined/3dshapes/data_attribution/das/01_train_datapoint_gradient.py" ]] && break
  candidate="$(dirname "${candidate}")"
done
REPO_ROOT="${candidate}"
[[ -f "${REPO_ROOT}/diffusion_jax_refined/3dshapes/data_attribution/das/01_train_datapoint_gradient.py" ]] || {
...
export DAS_AGGREGATE_MC_NORMALIZED=1
export DAS_AGGREGATE_MC_NORMALIZE_EPS="${DAS_AGGREGATE_MC_NORMALIZE_EPS:-1e-8}"
...
ARTIFACT_DIR="${SHAPES_ROOT}/result/${EXPERIMENT_TAG}/model/prompted_solo/seed_${TRAIN_SEED}_train_gradient/das_mc_normalized10x10"
...
echo "3D Shapes DAS: 10 timestamps x 10 noise-specific terms"
echo "Within each timestamp, gradients and residuals are jointly normalized across the 10 MC draws, then averaged"
