#!/usr/bin/env bash
#SBATCH -J 3d-pn24-trainrel
#SBATCH -o 3d-pn24-trainrel-%j.out
#SBATCH -e 3d-pn24-trainrel-%j.err
#SBATCH -p rtx-small
#SBATCH -N 1
#SBATCH -n 2
...
OUT_DIR="${SHAPES_ROOT}/result/${EXPERIMENT_TAG}/eval/probe24_nearest_train_predicted_noise/run_${SLURM_JOB_ID}"
mkdir -p "${OUT_DIR}"

echo "[phase 1/2] Q=${QUERY_IDS}: per-datapoint nearest probe vs current/next/delta predicted noise"
pids=()
for shard in 0 1; do
  (
...
echo "[done] nearest train-gradient probe/predicted-noise relation complete"
