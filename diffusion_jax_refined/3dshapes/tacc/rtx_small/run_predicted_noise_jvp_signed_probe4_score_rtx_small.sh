#!/usr/bin/env bash
#SBATCH -J 3d-pnoise-s4
#SBATCH -o 3d-pnoise-s4-%j.out
#SBATCH -e 3d-pnoise-s4-%j.err
#SBATCH -p rtx-small
#SBATCH -N 1
#SBATCH -n 2
...
LOG_ROOT="${SHAPES_ROOT}/result/${EXPERIMENT_TAG}/logs/predicted_noise_jvp_signed_probe4_score/${SLURM_JOB_ID}"
mkdir -p "${LOG_ROOT}"

echo "[phase 1/2] original train features x mean of four signed predicted-noise probe contractions"
pids=()
for shard in 0 1; do
  (
...
echo "[phase 2/2] cached LDS for four normalization variants"
JAX_PLATFORMS=cpu "${PYTHON_BIN}" "${SHAPES_ROOT}/script/run_traj_tracin_lds_cached.py" \
  --execute \
  --experiment "${EXPERIMENT_TAG}" \
  --train-seed "${TRAIN_SEED}" \
  --score-schemes predicted_noise_jvp_signed_probe4

echo "[done] score namespace=traj_tracin_predicted_noise_jvp_signed_probe4"
