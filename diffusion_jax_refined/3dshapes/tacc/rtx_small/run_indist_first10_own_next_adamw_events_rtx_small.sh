#!/usr/bin/env bash
#SBATCH -J 3d-id10-ownaw
#SBATCH -o 3d-id10-ownaw-%j.out
#SBATCH -e 3d-id10-ownaw-%j.err
#SBATCH -p rtx-small
#SBATCH -N 1
#SBATCH -n 2
...
query_namespace="loss_direction_original_f_checkpoint_own_trajectory_100t_indist_first10"
event_root="$shapes/result/$experiment/fixed_checkpoint_adamw_four_events_n5000"
out_root="$shapes/result/$experiment/eval/adamw_training_events_own_next100t_indist_first10/run_${SLURM_JOB_ID}"
...
cd "$shapes"
echo '[1/3] ID Q0-Q9: checkpoint-own trajectory next-checkpoint query gradients, 100 timestamps'
python script/run_traj_tracin_queries_and_scores.py \
  ...
  --num-snapshots 100 --log-prefix indist_first10_ownnext_adamw_events
...
echo "[done] ID first10 training-extracted AdamW events + own-next | $out_root"
