#!/usr/bin/env bash
#SBATCH -J 3d-pn12-2abs
#SBATCH -o 3d-pn12-2abs-%j.out
#SBATCH -e 3d-pn12-2abs-%j.err
#SBATCH -p rtx-small
#SBATCH -N 1
#SBATCH -n 2
...
NUM_PROBES=12
LOG_ROOT="${SHAPES_ROOT}/result/${EXPERIMENT_TAG}/logs/predicted_noise_old_fresh12_termwise_absolute/${SLURM_JOB_ID}"
mkdir -p "${LOG_ROOT}"
...
score_bank() {
  ...
  echo "[score ${label}] sqrt(product^2)=abs(product), then mean 12 probes"
  local pids=()
  for shard in 0 1; do
    (
...
echo "[LDS] old12 and fresh12; both prediction signs are emitted"
for sign in 1 -1; do
  JAX_PLATFORMS=cpu "${PYTHON_BIN}" "${SHAPES_ROOT}/script/run_traj_tracin_lds_cached.py" \
    ...
    --score-schemes \
      predicted_noise_jvp_absolute_probe12,predicted_noise_jvp_absolute_probe12_fresh_seed20260915 \
...
echo "[done] old12 and fresh12 termwise absolute LDS complete"
