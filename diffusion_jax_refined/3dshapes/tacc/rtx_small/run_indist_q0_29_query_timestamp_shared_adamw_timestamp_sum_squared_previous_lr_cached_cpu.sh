#!/usr/bin/env bash
#SBATCH -J 3d-q0-29-qtsl
#SBATCH -o 3d-q0-29-qtsl-%j.out
#SBATCH -e 3d-q0-29-qtsl-%j.err
#SBATCH -p skx-dev
#SBATCH -N 1
#SBATCH -n 1
...
residual_namespace="adamw_residual_aligned10x10_query_timestamp_shared_timestamp_sum_squared_previous_lr"
full_namespace="adamw_full_aligned10x10_query_timestamp_shared_timestamp_sum_squared_previous_lr"
...
cd "$shapes"
echo '[1/3] residual: checkpoint c uses stored LR from checkpoint c-1; checkpoint 0 weight is zero'
...
echo '[2/3] full AdamW with optimizer history and previous-checkpoint LR'
python script/run_traj_tracin_queries_and_scores.py \
  --execute --experiment "$experiment" --train-seed "$seed" \
  --query-file "$query_file" --query-ids "$query_ids" --gpus 0 \
  --skip-sampling --skip-query-gradient \
...
echo '[3/3] 960 cached LDS evaluations'
...
echo '[done] Q0-Q29 query-timestamp-shared timestamp-sum-square with previous-checkpoint LR'
