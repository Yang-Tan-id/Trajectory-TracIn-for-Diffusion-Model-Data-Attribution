#!/usr/bin/env bash
#SBATCH -J 3d-refdir10q
#SBATCH -o 3d-refdir10q-%j.out
#SBATCH -e 3d-refdir10q-%j.err
#SBATCH -p rtx-small
#SBATCH -N 1
#SBATCH -n 2
...
OUT_DIR="${SHAPES_ROOT}/result/${EXPERIMENT_TAG}/eval/reference_delta_direction_all_queries/run_${SLURM_JOB_ID}"
mkdir -p "${OUT_DIR}"

export TRAJ_TRACIN_PROBE_ALIGNMENT_ONLY=1
export TRAJ_TRACIN_PROBE_ALIGNMENT_COUNT=12
# This limits the current-checkpoint terms to c=1..49. The same forward pass
# also records Delta_ref=eps(reference)-eps(c); c+1 is only a comparison here.
export TRAJ_TRACIN_PROBE_ALIGNMENT_NEXT_CHECKPOINT=1

echo "[phase 1/4] current/reference Delta for original 12 probes | Q=${QUERY_IDS}"
python "${SHAPES_ROOT}/script/run_traj_tracin_queries_and_scores.py" \
  ...
  --log-prefix refdir10q_original12

echo "[phase 2/4] current/reference Delta for fresh 12 probes | Q=${QUERY_IDS}"
python "${SHAPES_ROOT}/script/run_traj_tracin_queries_and_scores.py" \
  --execute --experiment "${EXPERIMENT_TAG}" --train-seed "${TRAIN_SEED}" \
  --epochs "${JAX_EPOCHS}" --query-ids "${QUERY_IDS}" --gpus 0,1 \
  ...
  --num-snapshots 10 --log-prefix refdir10q_fresh12

unset TRAJ_TRACIN_PROBE_ALIGNMENT_ONLY
unset TRAJ_TRACIN_PROBE_ALIGNMENT_NEXT_CHECKPOINT

echo "[phase 3/4] select probe nearest to eps(reference)-eps(c)"
pids=()
for shard in 0 1; do
  (
    CUDA_VISIBLE_DEVICES="${shard}" JAX_NUM_DEVICES=1 JAX_PLATFORMS=cuda \
      "${PYTHON_BIN}" "${ANALYZER}" analyze-shard \
        ...
        --checkpoint-direction next --include-reference-delta
...
echo "[done] direct reference-Delta direction LDS complete"
