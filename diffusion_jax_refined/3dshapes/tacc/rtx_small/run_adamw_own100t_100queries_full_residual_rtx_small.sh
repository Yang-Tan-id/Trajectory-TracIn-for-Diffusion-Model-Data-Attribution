...
JAX_PLATFORMS=cpu python "$shapes/script/run_adamw_four_event_original_f_scores.py" \
  ...
  --query-file "$query_file" --query-ids "$ids" --query-namespace "$namespace" \
  --attribution-points 5000 --checkpoint-weighting uniform --contraction linear \
  --methods four,four_residual --out-dir "$out"
echo "[done] $out"
