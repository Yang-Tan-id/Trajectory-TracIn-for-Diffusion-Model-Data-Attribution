#!/usr/bin/env bash
#SBATCH -J 3d-q0-9-qts2
#SBATCH -o 3d-q0-9-qts2-%j.out
#SBATCH -e 3d-q0-9-qts2-%j.err
#SBATCH -p skx-dev
#SBATCH -N 1
#SBATCH -n 1
...
residual_namespace="adamw_residual_aligned10x10_query_timestamp_shared_termwise_squared"
full_namespace="adamw_full_aligned10x10_query_timestamp_shared_termwise_squared"
...
missing=0
for initial_seed in $(seq 100 109); do
  ...
  if ! find "$sample_root" -type f \
    ...
    echo "Missing query-timestamp-shared artifact for initial seed $initial_seed" >&2
    missing=1
  fi
done
[[ "$missing" == 0 ]] || exit 1
...
cd "$shapes"
echo '[1/3] residual AdamW: square each aligned checkpoint/timestamp dot product, then sum'
...
echo '[2/3] full AdamW termwise-square with optimizer history'
python script/run_traj_tracin_queries_and_scores.py \
  --execute --experiment "$experiment" --train-seed "$seed" \
  --query-file "$query_file" --query-ids "$query_ids" --gpus 0 \
  --skip-sampling --skip-query-gradient \
...
echo '[3/3] 320 cached LDS evaluations'
...
echo '[done] Q0-Q9 query-timestamp-shared AdamW termwise-square'
