#!/usr/bin/env bash
#SBATCH -J 3d-pnfix-groups
#SBATCH -o 3d-pnfix-groups-%j.out
#SBATCH -e 3d-pnfix-groups-%j.err
#SBATCH -p rtx-small
#SBATCH -N 1
#SBATCH -n 2
...
RUN_ID="${SLURM_JOB_ID}"
LOG_ROOT="${SHAPES_ROOT}/result/${EXPERIMENT_TAG}/logs/predicted_noise_fixed8_two_group/${SLURM_JOB_ID}"
mkdir -p "${LOG_ROOT}"

echo "[phase 1/2] retain all eight per-probe linear trajectory scores"
pids=()
for shard in 0 1; do
  (
    CUDA_VISIBLE_DEVICES="${shard}" JAX_NUM_DEVICES=1 JAX_PLATFORMS=cuda \
      "${PYTHON_BIN}" "${SCORE_DRIVER}" score-shard \
        ...
        --num-probes 8 --contraction final_post_square \
...
echo "[phase 2/2] print fixed probes 1-4 versus 5-8 for every query"
...
echo "[done] fixed two-group per-query LDS complete"
