#!/usr/bin/env bash
#SBATCH -J 3d-pn-prod2-ckpt
#SBATCH -o 3d-pn-prod2-ckpt-%j.out
#SBATCH -e 3d-pn-prod2-ckpt-%j.err
#SBATCH -p rtx-small
#SBATCH -N 1
#SBATCH -n 2
...
export NUM_PROBES="${NUM_PROBES:-8}"
...
LOG_ROOT="${SHAPES_ROOT}/result/${EXPERIMENT_TAG}/logs/predicted_noise_probe${NUM_PROBES}_product_square_per_checkpoint_lds/${SLURM_JOB_ID}"
mkdir -p "${LOG_ROOT}"

echo "[phase 1/2] retain E_timestamp[E_probe[(gradient product)^2]] for every checkpoint; probes=${NUM_PROBES}"
pids=()
for shard in 0 1; do
  (
    CUDA_VISIBLE_DEVICES="${shard}" JAX_NUM_DEVICES=1 JAX_PLATFORMS=cuda \
      "${PYTHON_BIN}" "${SCORE_DRIVER}" score-shard \
        ...
        --contraction squared --retain-checkpoint-scores \
...
echo "[phase 2/2] cached LDS independently for every checkpoint"
JAX_PLATFORMS=cpu "${PYTHON_BIN}" \
  "${SHAPES_ROOT}/script/analyze_predicted_noise_per_checkpoint_lds.py" \
  --experiment "${EXPERIMENT_TAG}" --train-seed "${TRAIN_SEED}" \
  --run-id "${SLURM_JOB_ID}" --num-probes "${NUM_PROBES}" \
  --contraction squared --prediction-sign=-1

echo "[done] Probe${NUM_PROBES} termwise product-square per-checkpoint LDS complete"
