#!/usr/bin/env bash
#SBATCH -J 3d-pn4o-termsq
#SBATCH -o 3d-pn4o-termsq-%j.out
#SBATCH -e 3d-pn4o-termsq-%j.err
#SBATCH -p rtx-small
#SBATCH -N 1
#SBATCH -n 2
...
LOG_ROOT="${SHAPES_ROOT}/result/${EXPERIMENT_TAG}/logs/predicted_noise_shared_orthogonal_probe4_termwise_square/${SLURM_JOB_ID}"
mkdir -p "${LOG_ROOT}"

echo "[phase 1/2] shared orthogonal probe4: square only each gradient dot product"
pids=()
for shard in 0 1; do
  (
...
JAX_PLATFORMS=cpu "${PYTHON_BIN}" "${SHAPES_ROOT}/script/run_traj_tracin_lds_cached.py" \
  ...
  --experiment "${EXPERIMENT_TAG}" \
  --train-seed "${TRAIN_SEED}" \
  --score-schemes predicted_noise_shared_orthogonal_probe4_termwise_square \
  --prediction-sign=-1
...
echo "[done] shared orthogonal probe4 termwise-square scores and LDS complete"
