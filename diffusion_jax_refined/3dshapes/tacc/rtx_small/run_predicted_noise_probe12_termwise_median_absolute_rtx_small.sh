#!/usr/bin/env bash
#SBATCH -J 3d-pn12-medabs
#SBATCH -o 3d-pn12-medabs-%j.out
#SBATCH -e 3d-pn12-medabs-%j.err
#SBATCH -p rtx-small
#SBATCH -N 1
#SBATCH -n 2
...
LOG_ROOT="${SHAPES_ROOT}/result/${EXPERIMENT_TAG}/logs/predicted_noise_probe12_termwise_median_absolute/${SLURM_JOB_ID}"
mkdir -p "${LOG_ROOT}"

echo "[phase 1/2] twelve probes: median absolute contraction within every checkpoint/timestamp"
pids=()
for shard in 0 1; do
  (
    CUDA_VISIBLE_DEVICES="${shard}" JAX_NUM_DEVICES=1 JAX_PLATFORMS=cuda \
      "${PYTHON_BIN}" "${SCORE_DRIVER}" score-shard \
        ...
        --num-probes "${NUM_PROBES}" --contraction median_absolute \
...
JAX_PLATFORMS=cpu "${PYTHON_BIN}" "${SHAPES_ROOT}/script/run_traj_tracin_lds_cached.py" \
  ...
  --score-schemes predicted_noise_jvp_median_absolute_probe12 --prediction-sign=-1
...
"${PYTHON_BIN}" "${SHAPES_ROOT}/script/print_predicted_noise_timestamp_checkpoint_square_lds.py" \
  ...
  --reduction median_absolute --prediction-sign m1

echo "[done] twelve-probe termwise median-absolute scores and LDS complete"
