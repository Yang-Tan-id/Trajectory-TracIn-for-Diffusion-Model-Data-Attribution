#!/usr/bin/env bash
#SBATCH -J 3d-refdelta10q
#SBATCH -o 3d-refdelta10q-%j.out
#SBATCH -e 3d-refdelta10q-%j.err
#SBATCH -p rtx-small
#SBATCH -N 1
#SBATCH -n 2
...
ORIGINAL_NAMESPACE="predicted_noise_output_next_reference_original12"
FRESH_NAMESPACE="predicted_noise_output_next_reference_fresh12"
OUT_DIR="${SHAPES_ROOT}/result/${EXPERIMENT_TAG}/eval/reference_oriented_delta_all_queries/run_${SLURM_JOB_ID}"
mkdir -p "${OUT_DIR}"
...
echo "[phase 1/4] current/next/reference predicted noise for original bank | Q=${QUERY_IDS}"
python "${SHAPES_ROOT}/script/run_traj_tracin_queries_and_scores.py" \
  ...
  --log-prefix refdelta10q_original12

echo "[phase 2/4] current/next/reference predicted noise for fresh bank | Q=${QUERY_IDS}"
python "${SHAPES_ROOT}/script/run_traj_tracin_queries_and_scores.py" \
  --execute --experiment "${EXPERIMENT_TAG}" --train-seed "${TRAIN_SEED}" \
  --epochs "${JAX_EPOCHS}" --query-ids "${QUERY_IDS}" --gpus 0,1 \
  ...
  --num-snapshots 10 --log-prefix refdelta10q_fresh12

unset TRAJ_TRACIN_PROBE_ALIGNMENT_ONLY
unset TRAJ_TRACIN_PROBE_ALIGNMENT_NEXT_CHECKPOINT

echo "[phase 3/4] score reference-oriented Delta selections"
pids=()
for shard in 0 1; do
  (
...
echo "[done] reference-oriented Delta LDS complete"
