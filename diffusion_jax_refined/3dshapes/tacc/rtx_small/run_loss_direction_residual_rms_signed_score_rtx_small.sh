#!/usr/bin/env bash
#SBATCH -J 3d-loss-rms-s
#SBATCH -o 3d-loss-rms-s-%j.out
#SBATCH -e 3d-loss-rms-s-%j.err
#SBATCH -p rtx-small
#SBATCH -N 1
#SBATCH -n 2
...
SCORE_PREFIX=traj_tracin_loss_direction_residual_rms_signed
LOG_ROOT="${SHAPES_ROOT}/result/${EXPERIMENT_TAG}/logs/loss_direction_residual_rms_signed_score/${SLURM_JOB_ID}"
mkdir -p "${LOG_ROOT}"

echo "[phase 1/2] signed predicted-noise score from retained train/query artifacts"
pids=()
for shard in 0 1; do
  (
...
echo "[phase 2/2] cached LDS for signed predicted-noise score"
JAX_PLATFORMS=cpu "${PYTHON_BIN}" "${SHAPES_ROOT}/script/run_traj_tracin_lds_cached.py" \
  --execute \
  --experiment "${EXPERIMENT_TAG}" \
  --train-seed "${TRAIN_SEED}" \
  --score-schemes loss_direction_residual_rms_predicted_noise_signed

echo "[done] signed predicted-noise scores and LDS are complete"
