#!/usr/bin/env bash
#SBATCH -J 3d-ref10-29-a10abs
#SBATCH -o 3d-ref10-29-a10abs-%j.out
#SBATCH -e 3d-ref10-29-a10abs-%j.err
#SBATCH -p skx-dev
#SBATCH -N 1
#SBATCH -n 1
...
export TRACIN_SCORE_CONTRACTION=absolute
...
residual_namespace="adamw_residual_aligned10x10_absolute"
full_namespace="adamw_full_aligned10x10_absolute"
...
cd "$shapes"
echo '[1/3] residual aligned10x10 termwise-absolute: crop cached Q10-Q29 reference 100t queries'
...
echo '[2/3] full AdamW aligned10x10 termwise-absolute with optimizer history'
python script/run_traj_tracin_queries_and_scores.py \
  --execute --experiment "$experiment" --train-seed "$seed" \
  --query-file "$query_file" --query-ids "$query_ids" --gpus 0 \
  --skip-sampling --skip-query-gradient \
...
echo '[3/3] cached LDS for absolute residual/full and four normalization variants'
...
echo '[done] Q10-Q29 reference-next AdamW aligned10x10 termwise-absolute'
