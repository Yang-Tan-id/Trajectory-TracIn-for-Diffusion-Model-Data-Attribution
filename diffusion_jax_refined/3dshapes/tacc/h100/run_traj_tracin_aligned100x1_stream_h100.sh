...
echo "3D Shapes aligned 100 timestamps x 1 MC saved-gradient Traj TracIn"
echo "nodes=4 gpus=16 queries=10 attribution_points=5000 checkpoints=49"
echo "query gradients are cached first; train gradients are retained as 49 checkpoint parts"
echo "estimated train-gradient storage: approximately 374 GiB"
echo "repo=${REPO_ROOT} experiment=${EXPERIMENT_TAG} train_seed=${TRAIN_SEED}"
echo "logs=${LOG_ROOT}"

echo "[phase 1/4] query gradients: 10 queries on slots 0-9"
query_pids=()
...
echo "[phase 1/4] all 10 query-gradient artifacts complete"

echo "[phase 2/4] saved train gradients: 16 checkpoint shards"
train_pids=()
for slot in $(seq 0 15); do
  gpu="$((slot % GPU_PER_NODE))"
  echo "[train launch] checkpoint_shard=${slot}/16 slot=${slot} local_gpu=${gpu}"
  (
    run_slot "${slot}" \
...
wait_all "saved-train-gradient" "${train_pids[@]}"
echo "[phase 2/4] all 16 train checkpoint shards complete"

echo "[phase 2/4] validate 49 saved checkpoint parts (no 374-GiB duplicate merge)"
"${PYTHON_BIN}" "${DRIVER}" validate-train \
  --experiment "${EXPERIMENT_TAG}" \
  --train-seed "${TRAIN_SEED}" \
  --epochs "${JAX_EPOCHS}" \
  --python-bin "${PYTHON_BIN}"

echo "[phase 3/4] score all 10 queries from saved parts on 16 checkpoint shards"
score_pids=()
for slot in $(seq 0 15); do
  gpu="$((slot % GPU_PER_NODE))"
  echo "[score launch] checkpoint_shard=${slot}/16 slot=${slot} local_gpu=${gpu}"
  (
    run_slot "${slot}" \
      "${PYTHON_BIN}" "${DRIVER}" score-saved \
      --experiment "${EXPERIMENT_TAG}" \
      --train-seed "${TRAIN_SEED}" \
      --epochs "${JAX_EPOCHS}" \
      --shard-index "${slot}" \
...
wait_all "saved-gradient-score" "${score_pids[@]}"
echo "[phase 3/4] all 16 saved-gradient score shards complete"

echo "[phase 4/4] merge score shards and materialize 4 variants x 10 queries"
"${PYTHON_BIN}" "${DRIVER}" merge-saved \
  --experiment "${EXPERIMENT_TAG}" \
  --train-seed "${TRAIN_SEED}" \
  --epochs "${JAX_EPOCHS}" \
  --shard-count 16 \
...
echo "[done] train-gradient namespace=traj_tracin_aligned100x1_saved"
echo "[done] checkpoint parts retained; merged train artifact intentionally omitted"
echo "[done] score namespace=traj_tracin_aligned100x1_saved"
