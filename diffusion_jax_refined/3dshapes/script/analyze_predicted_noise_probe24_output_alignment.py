...
def load_bank(args: argparse.Namespace, query_id: int, bank: str) -> dict[tuple[int, int, int], dict[str, float]]:
    ...
    with np.load(path, allow_pickle=False) as payload:
        ...
        reference_direction_delta = (
            np.asarray(
                payload["reference_direction_delta_probe_cosines"],
                dtype=np.float64,
            )
            if "reference_direction_delta_probe_cosines" in payload
            else None
        )
        reference_direction_delta_scalar = (
            np.asarray(
                payload["reference_direction_delta_probe_scalars"],
                dtype=np.float64,
            )
            if "reference_direction_delta_probe_scalars" in payload
            else None
        )
        future_mean_delta = (
            np.asarray(
                payload["future_mean_delta_probe_cosines"], dtype=np.float64
            )
            if "future_mean_delta_probe_cosines" in payload
            else None
        )
        future_mean_delta_scalar = (
            np.asarray(
                payload["future_mean_delta_probe_scalars"], dtype=np.float64
            )
            if "future_mean_delta_probe_scalars" in payload
            else None
        )
        future_lr_delta = (
            np.asarray(
                payload["future_lr_weighted_delta_probe_cosines"],
                dtype=np.float64,
            )
            if "future_lr_weighted_delta_probe_cosines" in payload
            else None
        )
        future_lr_delta_scalar = (
            np.asarray(
                payload["future_lr_weighted_delta_probe_scalars"],
                dtype=np.float64,
            )
    ...
    for term, (ckpt, timestep) in enumerate(zip(ckpts, timesteps)):
        for probe in range(current.shape[0]):
            ...
            if reference_direction_delta is not None:
                values["cosine_to_reference_predicted_noise_direction_delta"] = float(
                    reference_direction_delta[probe, term]
                )
            if reference_direction_delta_scalar is not None:
                values[
                    "projection_on_reference_predicted_noise_direction_delta"
                ] = float(reference_direction_delta_scalar[probe, term])
            if future_mean_delta is not None:
                values["cosine_to_future_mean_predicted_noise_delta"] = float(
                    future_mean_delta[probe, term]
                )
            if future_mean_delta_scalar is not None:
                values["projection_on_future_mean_predicted_noise_delta"] = float(
                    future_mean_delta_scalar[probe, term]
                )
            if future_lr_delta is not None:
                values["cosine_to_future_lr_weighted_predicted_noise_delta"] = float(
                    future_lr_delta[probe, term]
                )
            if future_lr_delta_scalar is not None:
                values[
