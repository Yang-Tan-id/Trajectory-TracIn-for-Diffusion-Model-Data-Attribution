...
REFERENCE_CKPT = str(PROMPTED_JAX_MODEL_ROOT / "seed_0_epoch_0100.ckpt")
ATTRIBUTION_SAMPLE_DIR = str(SAMPLING_ROOT / "artbench_latent" / "prompt_baroque" / "model_prompted_jax__ckpt_seed_0_epoch_0100")
...
def commands_for_algorithm(algorithm: str) -> dict[str, list[str]]:
    ...
    run_name = f"ArtBench-unprompted-{algorithm}-sub{subset_index}-seed{seed}"
    return {
        "training": [
            "python",
            ...
            f"--dataset_name_or_path={DATASET_STORAGE_ROOT / 'hf_artbench'}",
            f"--index_path={DATASET_STORAGE_ROOT / 'indices' / f'sub-idx-{subset_index}.pkl'}",
