#!/usr/bin/env bash
#SBATCH --job-name=cifar2-lds-4modes
#SBATCH --partition=gh
...
#SBATCH --output=cifar2-lds-4modes-%j.out
#SBATCH --error=cifar2-lds-4modes-%j.err
...
LOG_ROOT="${CIFAR2_ROOT}/result/${EXPERIMENT_TAG}/vista_logs/01_train_lds_models/${SLURM_JOB_ID:-local}"
mkdir -p "${LOG_ROOT}"
