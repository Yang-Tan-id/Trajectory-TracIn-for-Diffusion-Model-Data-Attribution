...
For `das`, `traj_tracin`, `dtrak`, and `end_tracin`, the query used by
attribution is a saved generated sample, not a new sample created during
attribution. Its identity is determined by three fields in the selected
...
`QUERY` selects the conditional prompt and therefore the sampling folder.
`attribution_sample_seed` selects one of the seeds generated by
`SAMPLE_SEEDS`, and `attribution_sample_index` selects an item within that
seed's saved batch. The sample must be generated before attribution starts.

The checked-in CIFAR2 configuration currently uses sample seed `1`, index `0`
for `das` and `traj_tracin`; `dtrak` and `end_tracin` still use seed `0`,
index `0`. Check the target algorithm entry before launching a run.

`journey_trak` is currently an exception: it uses `QUERY` as conditioning and
constructs its query trajectory inside the attribution engine. It does not
select a saved sample with `attribution_sample_seed` and
`attribution_sample_index`.
...
EXPERIMENT_TAG=experiment1 QUERY=horse SAMPLE_SEEDS=0,1,2 CUDA_VISIBLE_DEVICES=0 bash scripts/00_sample.sh
...
```text
result/experiment1/eval/sampling/cifar/prompt_horse/model_prompted_jax__ckpt_seed_42_epoch_0200/
```

For `das`, `traj_tracin`, `dtrak`, and `end_tracin`,
`01_data_attribution.sh` only consumes the saved query sample and trajectory;
it does not run sampling automatically. `journey_trak` follows the exception
described above.
