#!/usr/bin/env bash
#SBATCH -J cifar2-s3-traj-attr-all
#SBATCH -o cifar2-s3-traj-attr-all-%j.out
#SBATCH -e cifar2-s3-traj-attr-all-%j.err
#SBATCH -p h100
#SBATCH -N 4
#SBATCH -n 16
...
LOG_ROOT="${CIFAR2_ROOT}/result/${EXPERIMENT_TAG}/stampede3_das_logs/02_traj_tracin_attribution_all/${SLURM_JOB_ID:-local}"
mkdir -p "${LOG_ROOT}"
  ...
  if [[ "${unprompted_flag}" == "1" || "${score_mode}" == unprompted_* ]]; then
    if [[ "${variant}" == "normalized" ]]; then
      printf '%s/result/%s/attribution_score/%s/train_seed_%s/unprompted/initial_seed_%s/traj_tracin_normalized_unprompted_%s/scores.npy' \
        "${CIFAR2_ROOT}" "${EXPERIMENT_TAG}" "${score_mode}" "${TRAIN_SEED}" "${seed}" "${tag}"
    else
    ...
    if [[ "${variant}" == "normalized" ]]; then
      printf '%s/result/%s/attribution_score/%s/train_seed_%s/query_%s/initial_seed_%s/traj_tracin_normalized_%s/scores.npy' \
        "${CIFAR2_ROOT}" "${EXPERIMENT_TAG}" "${score_mode}" "${TRAIN_SEED}" "$(path_tag "${query}")" "${seed}" "${tag}"
    else
