...
case "${stage}" in
  base_lds)
    ...
    echo "  STAMPEDE3_ACCOUNT=${STAMPEDE3_ACCOUNT:-${ACCOUNT:-}} EXPERIMENT_TAG=${EXPERIMENT_TAG:-experiment_67} TRAIN_SEED=${TRAIN_SEED:-67} STAMPEDE3_SUBMIT_STAGE=attr bash diffusion_jax_refined/cifar2/stampede3_das/submit_stampede3_das_pipeline.sh"
    echo
  ...
  attr)
    ...
    echo "  STAMPEDE3_ACCOUNT=${STAMPEDE3_ACCOUNT:-${ACCOUNT:-}} EXPERIMENT_TAG=${EXPERIMENT_TAG:-experiment_67} TRAIN_SEED=${TRAIN_SEED:-67} STAMPEDE3_SUBMIT_STAGE=eval LDS_JOB_ID=<01_job_id> ATTR_JOB_IDS=${attr_jobs} bash diffusion_jax_refined/cifar2/stampede3_das/submit_stampede3_das_pipeline.sh"
    echo
    echo "Progress commands:"
