...
# It scores a configurable query set, then fast-evaluates against existing
# cached LDS target CSVs.
...
FULL_DIRECT_VARIANTS="${FULL_DIRECT_VARIANTS:-raw q_l2}"
...
FULL_DIRECT_LOG_NAME="${FULL_DIRECT_LOG_NAME:-full_traj_direct_4query}"

# Either score_mode|query|seed|unprompted_flag or
# sample_mode|score_mode|query|seed|unprompted_flag.
...
alg_base_for() {
  ...
  if [[ "${unprompted}" == "1" ]]; then
    prefix='traj_tracin_full_direct_avg100mc_nextckpt_raw_unprompted'
  else
    prefix='traj_tracin_full_direct_avg100mc_nextckpt_raw'
  fi
  if [[ "${variant}" == "q_l2" || "${variant}" == "query_l2" || "${variant}" == "query_l2_normalized" ]]; then
    prefix="${prefix/traj_tracin/traj_tracin_normalized}"
  fi
  printf '%s' "${prefix}"
}
...
score_dir_for() {
  ...
  local variant="${6:-raw}"
  local alg_base alg
  alg_base="$(alg_base_for "${unprompted}" "${variant}")"
...
run_one() {
  local task_id="$1" gpu="$2" sample_mode="$3" score_mode="$4" query="$5" seed="$6" unprompted="$7" range="$8"
  local out_dir normalized_out_dir alg query_env log_root log
  out_dir="$(score_dir_for "${score_mode}" "${query}" "${seed}" "${unprompted}" "${range}")"
  normalized_out_dir="$(score_dir_for "${score_mode}" "${query}" "${seed}" "${unprompted}" "${range}" q_l2)"
  if [[ -f "${out_dir}/scores.npy" && ( "${TRAJ_SAVE_QUERY_NORMALIZED_SCORES}" != "1" || -f "${normalized_out_dir}/scores.npy" ) ]]; then
    echo "[skip-score] ${out_dir}/scores.npy"
    if [[ "${TRAJ_SAVE_QUERY_NORMALIZED_SCORES}" == "1" ]]; then
      echo "[skip-score] ${normalized_out_dir}/scores.npy"
    fi
    return 0
  fi
  ...
  query_env="${query}"
  [[ "${unprompted}" == "1" ]] && query_env="unconditional"
  log_root="${CIFAR2_ROOT}/result/${EXPERIMENT_TAG}/local_logs/${FULL_DIRECT_LOG_NAME}"
  mkdir -p "${log_root}"
  log="${log_root}/task_${task_id}__${score_mode}__$(qtag "${query}")__seed_${seed}__$(rtag "${range}").log"
  echo "[run-score] gpu=${gpu} sample_mode=${sample_mode} score_mode=${score_mode} query=${query} seed=${seed} range=${range} -> ${log}"
  (
    cd "${REPO_ROOT}"
    CUDA_VISIBLE_DEVICES="${gpu}" \
    EXPERIMENT_TAG="${EXPERIMENT_TAG}" TRAIN_SEED="${TRAIN_SEED}" \
    SAMPLE_MODEL_MODE="${sample_mode}" ATTRIBUTION_SAMPLE_MODEL_MODE="${sample_mode}" UNPROMPTED_SAMPLE_MODEL_MODE="${sample_mode}" \
...
eval_one_group() {
  local score_mode="$1" query="$2" seed="$3" unprompted="$4" variant="$5"
  local alg_base score_dirs mode_arg eval_root target lds_seed target_csv out_dir summary
  alg_base="$(alg_base_for "${unprompted}" "${variant}")"
  score_dirs=""
  for range in ${RANGES}; do
    local dir
    dir="$(score_dir_for "${score_mode}" "${query}" "${seed}" "${unprompted}" "${range}" "${variant}")"
    [[ -f "${dir}/scores.npy" ]] || return 0
    score_dirs="${score_dirs:+${score_dirs},}${dir}"
  done
  if [[ "${unprompted}" == "1" ]]; then
    mode_arg="unprompted"
    eval_root="${CIFAR2_ROOT}/result/${EXPERIMENT_TAG}/eval/${score_mode}/unprompted/initial_seed_${seed}/lds_unprompted/${alg_base}"
  else
    mode_arg="prompted"
    eval_root="${CIFAR2_ROOT}/result/${EXPERIMENT_TAG}/eval/${score_mode}/query_$(qtag "${query}")/initial_seed_${seed}/lds/${alg_base}"
  fi
  for target in ${LDS_TARGETS}; do
    for lds_seed in ${LDS_SEEDS}; do
      target_csv="$(find_target_csv "${score_mode}" "${query}" "${seed}" "${unprompted}" "${target}" "${lds_seed}")"
...
summarize_group() {
  ...
  alg_base="$(alg_base_for "${unprompted}" "${variant}")"
  if [[ "${unprompted}" == "1" ]]; then
    root="${CIFAR2_ROOT}/result/${EXPERIMENT_TAG}/eval/${score_mode}/unprompted/initial_seed_${seed}/lds_unprompted/${alg_base}"
  else
    root="${CIFAR2_ROOT}/result/${EXPERIMENT_TAG}/eval/${score_mode}/query_$(qtag "${query}")/initial_seed_${seed}/lds/${alg_base}"
  fi
  "${PYTHON_BIN}" - "${score_mode}" "${query}" "${seed}" "${alg_base}" "${root}" <<'PY'
import json, sys
...
echo "Full direct Traj-TracIn query-set run"
...
for spec in ${QUERY_SPECS}; do
  ...
  if (( ${#fields[@]} == 5 )); then
    ...
    echo "Bad QUERY_SPECS entry: ${spec}" >&2
    exit 2
  fi
  for range in ${RANGES}; do
    tasks+=("${sample_mode}|${score_mode}|${query}|${seed}|${unprompted}|${range}")
  done
done

if [[ "${FULL_DIRECT_SRUN_WORKER:-0}" == "1" ]]; then
  worker_index="${FULL_DIRECT_WORKER_INDEX:-${SLURM_PROCID:-0}}"
  worker_count="${FULL_DIRECT_WORKER_COUNT:-${SLURM_NTASKS:-1}}"
  gpu_per_node="${GPU_PER_NODE:-4}"
  local_rank="${SLURM_LOCALID:-$((worker_index % gpu_per_node))}"
  gpu=$((local_rank % gpu_per_node))
  echo "[worker] index=${worker_index}/${worker_count} local_rank=${local_rank} gpu=${gpu} tasks=${#tasks[@]}"
  for ((i = worker_index; i < ${#tasks[@]}; i += worker_count)); do
    IFS='|' read -r sample_mode score_mode query seed unprompted range <<<"${tasks[$i]}"
    run_one "${i}" "${gpu}" "${sample_mode}" "${score_mode}" "${query}" "${seed}" "${unprompted}" "${range}"
  done
  exit 0
fi

if [[ "${FULL_DIRECT_EVAL_ONLY:-0}" != "1" ]]; then
  pids=()
  for i in "${!tasks[@]}"; do
    IFS='|' read -r sample_mode score_mode query seed unprompted range <<<"${tasks[$i]}"
    gpu=$(( i % GPU_SLOTS ))
...
for spec in ${QUERY_SPECS}; do
  ...
  for variant in ${FULL_DIRECT_VARIANTS}; do
    eval_one_group "${score_mode}" "${query}" "${seed}" "${unprompted}" "${variant}"
    summarize_group "${score_mode}" "${query}" "${seed}" "${unprompted}" "${variant}"
  done
done
