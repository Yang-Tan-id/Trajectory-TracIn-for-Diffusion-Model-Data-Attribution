...
def extended_shared_orthogonal_predicted_noise_probes(
    seed: int,
    output_shape: Sequence[int],
    probe_count: int,
    base_probe_count: int = 4,
):
    """Extend the historical fixed bank without changing its first probes."""
    output_shape = tuple(int(value) for value in output_shape)
    output_size = int(np.prod(output_shape))
    if not 0 < base_probe_count < probe_count <= output_size:
        raise ValueError(
            "expected 0 < base_probe_count < probe_count <= output size, got "
            f"base={base_probe_count}, count={probe_count}, size={output_size}"
        )
    scale = jnp.sqrt(jnp.asarray(output_size, dtype=jnp.float32))
    base = shared_orthogonal_predicted_noise_probes(
        seed, output_shape, base_probe_count
    ).reshape((base_probe_count, output_size))
    base_unit = base / scale

    key = jax.random.fold_in(jax.random.PRNGKey(seed), 0x45585444)
    candidates = jax.random.normal(
        key,
        (output_size, probe_count - base_probe_count),
        dtype=jnp.float32,
    )
    # Remove every component in the historical bank, then orthogonalize only
    # the new columns. Thus probes 0..3 remain bit-for-bit generated by the
    # original four-probe routine.
    candidates = candidates - base_unit.T @ (base_unit @ candidates)
    added, triangular = jnp.linalg.qr(candidates, mode="reduced")
    signs = jnp.where(jnp.diag(triangular) < 0, -1.0, 1.0)
    added = (added * signs[None, :]).T * scale
    return jnp.concatenate((base, added), axis=0).reshape(
        (probe_count,) + output_shape
    )
...
def run_attribution(cfg: TrajAttributionConfig):
    ...
    if (
        predicted_noise_probe_mode in ("shared_orthogonal", "shared_orthogonal_extended")
    ...
    def next_checkpoint_params(ckpt_i: int):
    ...
    if stage_mode:
        ...
        probe_alignment_next_checkpoint = (
            probe_alignment_only
            and os.environ.get(
                "TRAJ_TRACIN_PROBE_ALIGNMENT_NEXT_CHECKPOINT", "0"
            ).strip().lower()
            in ("1", "true", "yes", "on")
        )
        ...
        probe_alignment_next_scalars = []
        probe_alignment_next_cosines = []
                    ...
                    if uses_checkpoint_trajectory_target:
                        ...
                        if probe_alignment_only:
                            ...
                            if probe_alignment_next_checkpoint:
                                ...
                                def alignment_values(output):
                                    ...
                                    output_scalar = output_dot / jnp.sqrt(
                                        jnp.asarray(
                                            output_dimension, dtype=jnp.float32
                                        )
                                    )
                                    output_cosine = output_dot / jnp.maximum(
                                        probe_norm * output_norm[None, :],
                                        jnp.asarray(1e-12, dtype=jnp.float32),
                                    )
                                ...
                                probe_alignment_next_cosines.append(
                                    np.asarray(
                                        jax.device_get(next_cosine), dtype=np.float32
                                    )
                                )
                                probe_alignment_next_eps_norms.append(
                                    np.asarray(
                                        jax.device_get(next_norm), dtype=np.float32
                                    )
                                )
                                probe_alignment_delta_scalars.append(
                                    np.asarray(
                                        jax.device_get(delta_scalar), dtype=np.float32
                                    )
                                )
                                probe_alignment_delta_cosines.append(
                                    np.asarray(
                                        jax.device_get(delta_cosine), dtype=np.float32
                                    )
                                )
                            ...
                            query_total_terms = stage_checkpoint_count * len(t_seq)
                            print(
                        ...
                        if predicted_noise_probe_mode in (
                            ...
                            if shared_orthogonal_probe_bank is None:
                                probe_factory = (
                                    extended_shared_orthogonal_predicted_noise_probes
                                    if predicted_noise_probe_mode
                                    == "shared_orthogonal_extended"
                                    else shared_orthogonal_predicted_noise_probes
                                )
            ...
            if uses_predicted_noise_probe:
                query_payload.update(
                    output_probe_distribution=np.asarray(
                        "orthogonal_gaussian_qr_extended_from_historical_four"
                    ...
                    output_probe_seed_rule=np.asarray(
                        ...
                        "projected off the first four and QR-orthogonalized"
                        if predicted_noise_probe_mode == "shared_orthogonal_extended"
