...
#SBATCH -o x3-s3-orig-attr-%A_%a.out
#SBATCH -e x3-s3-orig-attr-%A_%a.err
#SBATCH -p h100
#SBATCH -N 4
#SBATCH -n 16
...
if [[ -z "${SCRIPT_DIR}" || ! -f "${SCRIPT_DIR}/02_dtrak_endtracin_attribution_chunk_stampede3.sh" ]]; then
  for candidate in "${SLURM_SUBMIT_DIR:-}" "${SLURM_SUBMIT_DIR:-}/stampede3_das" "${SLURM_SUBMIT_DIR:-}/diffusion_jax_refined/x3/stampede3_das"; do
