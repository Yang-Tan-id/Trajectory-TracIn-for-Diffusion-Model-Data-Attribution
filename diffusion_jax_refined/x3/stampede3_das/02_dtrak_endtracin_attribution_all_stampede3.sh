#!/usr/bin/env bash
#SBATCH -J x3-s3-orig-attr-all
#SBATCH -o x3-s3-orig-attr-all-%j.out
#SBATCH -e x3-s3-orig-attr-all-%j.err
#SBATCH -p h100
#SBATCH -N 4
#SBATCH -n 16
...
LOG_ROOT="${X3_ROOT}/result/${EXPERIMENT_TAG}/stampede3_das_logs/02_dtrak_endtracin_attribution_all/${SLURM_JOB_ID:-local}"
mkdir -p "${LOG_ROOT}"
