#!/usr/bin/env bash
#SBATCH -J x3-s3-das-smoke
#SBATCH -o x3-s3-das-smoke-%j.out
#SBATCH -e x3-s3-das-smoke-%j.err
#SBATCH -p h100
#SBATCH -N 1
#SBATCH -n 1
...
SMOKE_DAS_TIMESTEPS="${SMOKE_DAS_TIMESTEPS:-0}"
SMOKE_DAS_NUM_MC_NOISE="${SMOKE_DAS_NUM_MC_NOISE:-1}"
JAX_EPOCHS="${JAX_EPOCHS:-200}"
export DAS_DAMPING_SWEEP DAS_DAMPING_SWEEP_VALUES

LOG_ROOT="${X3_ROOT}/result/${EXPERIMENT_TAG}/stampede3_das_logs/02_smoke_das_attribution/${SLURM_JOB_ID:-local}"
mkdir -p "${LOG_ROOT}"
...
sample_run_root="${X3_ROOT}/result/${EXPERIMENT_TAG}/sample/x3/prompt_${prompt_tag}/model_${SMOKE_SAMPLE_MODE}__ckpt_${ckpt_stem}"
...
echo "experiment=${EXPERIMENT_TAG}; train_seed=${TRAIN_SEED}; sample_mode=${SMOKE_SAMPLE_MODE}; score_mode=${SMOKE_SCORE_MODE}; query=${SMOKE_QUERY}; seed=${SMOKE_INITIAL_SEED}; lambdas=${DAS_DAMPING_SWEEP_VALUES}; ranges=${SMOKE_SCORE_INDEX_RANGES}; proj_dim=${SMOKE_DAS_PROJ_DIM}; timesteps=${SMOKE_DAS_TIMESTEPS}; mc=${SMOKE_DAS_NUM_MC_NOISE}"
echo "logs=${LOG_ROOT}"
...
run_gpu_slot 0 env \
  ...
  DAS_TIMESTEPS="${SMOKE_DAS_TIMESTEPS}" \
  DAS_NUM_MC_NOISE="${SMOKE_DAS_NUM_MC_NOISE}" \
  X3_ROOT="${X3_ROOT}" \
...
for lambda in ${DAS_DAMPING_SWEEP_VALUES}; do
  ...
  if [[ "${SMOKE_UNPROMPTED}" == "1" || "${SMOKE_SCORE_MODE}" == unprompted_* ]]; then
    score_file="${X3_ROOT}/result/${EXPERIMENT_TAG}/attribution_score/${SMOKE_SCORE_MODE}/train_seed_${TRAIN_SEED}/unprompted/initial_seed_${SMOKE_INITIAL_SEED}/das_unprompted/lambda_${lambda_tag}/scores.npy"
  else
    score_file="${X3_ROOT}/result/${EXPERIMENT_TAG}/attribution_score/${SMOKE_SCORE_MODE}/train_seed_${TRAIN_SEED}/query_$(path_tag "${SMOKE_QUERY}")/initial_seed_${SMOKE_INITIAL_SEED}/das/lambda_${lambda_tag}/scores.npy"
  fi
