...
stampede3_das_init() {
  ...
  X3_ROOT="${X3_ROOT:-${REPO_ROOT}/diffusion_jax_refined/x3}"
  ...
  export REPO_ROOT X3_ROOT REFINE_ROOT EXPERIMENT_TAG TRAIN_SEED PYTHON_BIN
  ...
  [[ -d "${X3_ROOT}" ]] || {
    echo "X3 root not found: ${X3_ROOT}" >&2
    exit 1
  }
