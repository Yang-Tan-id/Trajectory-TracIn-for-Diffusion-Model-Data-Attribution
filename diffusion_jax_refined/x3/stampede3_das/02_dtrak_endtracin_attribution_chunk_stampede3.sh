#!/usr/bin/env bash
#SBATCH -J x3-s3-orig-attr
#SBATCH -o x3-s3-orig-attr-%j.out
#SBATCH -e x3-s3-orig-attr-%j.err
#SBATCH -p h100
#SBATCH -N 4
#SBATCH -n 16
...
LOG_ROOT="${X3_ROOT}/result/${EXPERIMENT_TAG}/stampede3_das_logs/02_dtrak_endtracin_attribution/${SLURM_JOB_ID:-local}/chunk_${ATTR_JOB_INDEX}"
mkdir -p "${LOG_ROOT}"
...
expected_score_file() {
  ...
  if [[ "${unprompted_flag}" == "1" || "${score_mode}" == unprompted_* ]]; then
    printf '%s/result/%s/attribution_score/%s/train_seed_%s/unprompted/initial_seed_%s/%s_unprompted/scores.npy' \
      "${X3_ROOT}" "${EXPERIMENT_TAG}" "${score_mode}" "${TRAIN_SEED}" "${seed}" "${algorithm}"
  else
    printf '%s/result/%s/attribution_score/%s/train_seed_%s/query_%s/initial_seed_%s/%s/scores.npy' \
      "${X3_ROOT}" "${EXPERIMENT_TAG}" "${score_mode}" "${TRAIN_SEED}" "$(path_tag "${query}")" "${seed}" "${algorithm}"
  fi
}
