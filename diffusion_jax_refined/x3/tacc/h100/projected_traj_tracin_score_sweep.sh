...
PROJECTED_DIMS="${PROJECTED_DIMS:-2048,4096,8192,16384,32768}"
...
export REPO_ROOT X3_ROOT REFINE_ROOT EXPERIMENT_TAG TRAIN_SEED QUERY INITIAL_SEED SAMPLE_MODEL_MODE
...
default_sample_dir() {
  if is_unprompted_mode; then
    printf '%s/result/%s/sample/x3/prompt_unconditional/model_%s__ckpt_seed_%s_epoch_%04d' \
      "${X3_ROOT}" "${EXPERIMENT_TAG}" "${SAMPLE_MODEL_MODE}" "${TRAIN_SEED}" "${JAX_EPOCHS:-200}"
  else
    printf '%s/result/%s/sample/x3/prompt_%s/model_%s__ckpt_seed_%s_epoch_%04d' \
      "${X3_ROOT}" "${EXPERIMENT_TAG}" "$(path_tag "${QUERY}")" "${SAMPLE_MODEL_MODE}" "${TRAIN_SEED}" "${JAX_EPOCHS:-200}"
  fi
}
...
ARTIFACT_BASE="${PROJECTED_ARTIFACT_BASE:-${X3_ROOT}/result/${EXPERIMENT_TAG}/projected_traj_tracin_artifacts/${ATTRIBUTION_SCORE_MODEL_MODE}/train_seed_${TRAIN_SEED}}"
...
SCORE_OUT_ROOT="${SCORE_OUT_ROOT:-${X3_ROOT}/result/${EXPERIMENT_TAG}/attribution_score/${ATTRIBUTION_SCORE_MODEL_MODE}/train_seed_${TRAIN_SEED}/${QCOMP}/initial_seed_${INITIAL_SEED}/${SCORE_ALGORITHM_DIR}}"
