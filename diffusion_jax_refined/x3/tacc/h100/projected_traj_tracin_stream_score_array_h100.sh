#!/usr/bin/env bash
#SBATCH -J x3-proj-traj-stream
#SBATCH -p h100
#SBATCH -N 4
#SBATCH -n 16
#SBATCH -t 24:00:00
#SBATCH -o x3-proj-traj-stream-%j.out
#SBATCH -e x3-proj-traj-stream-%j.err
...
query_artifacts_for_score_mode() {
  local score_mode="$1"
  local artifact_base="${PROJECTED_ARTIFACT_BASE:-${X3_ROOT}/result/${EXPERIMENT_TAG}/projected_traj_tracin_artifacts/${score_mode}/train_seed_${TRAIN_SEED}}"
...
run_one_score_mode() {
  ...
  artifact_base="${PROJECTED_ARTIFACT_BASE:-${X3_ROOT}/result/${EXPERIMENT_TAG}/projected_traj_tracin_artifacts/${score_mode}/train_seed_${TRAIN_SEED}}"
  dims_tag="$(path_tag "${STREAM_PROJ_DIMS}")"
  if [[ -n "${STREAM_SAVE_TERM_SCORE_VARIANTS}" ]]; then
    default_stream_root="${artifact_base}/stream_term_scores/cache_${PROJECTED_CACHE_DIM}/proj_${dims_tag}/variants_$(path_tag "${STREAM_SAVE_TERM_SCORE_VARIANTS}")"
  else
