...
attribution_results/traj_tracein/cifar2_traj_attr_cifar10_horse_automobile_from_samplee_model_horse_automobile__ckpt_seed_0_epoch_0200_horse_automobile_range_7501_10000" \
  ...
  --simple-loss-timestep 0 \
  --simple-loss-noise-seed 0 \
  --run-tag simple_loss \
  --epochs 200 \
  --prefer-device gpu \
  --num-devices 1 \
...
class CifarTargetEvaluator:
    def __init__(
        ...
        simple_loss_timestep: int,
        simple_loss_noise_seed: int,
    ):
        ...
        self.simple_loss_timestep = int(simple_loss_timestep)
        self.simple_loss_noise_seed = int(simple_loss_noise_seed)
    ...
    def _simple_loss_objective(self, target_params, x0_ref, t_scalar, rng):
        x0 = x0_ref[0]
    ...
    def evaluate(self, checkpoint: str) -> Tuple[float, Dict[str, object]]:
        ...
        if self.target_function == "noise_trajectory":
            ...
            t_scalar = jax.device_put(jnp.asarray(self.simple_loss_timestep, dtype=jnp.int32), self.device)
            rng = jax.device_put(jax.random.PRNGKey(self.simple_loss_noise_seed), self.device)
            value = self._simple_loss_fn(target_params, self.xt_ref, t_scalar, rng)
            details = {
                "target_function": "simple_loss",
                "simple_loss_timestep": int(self.simple_loss_timestep),
                "simple_loss_noise_seed": int(self.simple_loss_noise_seed),
            }
        else:
...
def main():
    ...
    evaluator = CifarTargetEvaluator(
        ...
        trajectory_reduction=args.trajectory_reduction,
        simple_loss_timestep=int(args.simple_loss_timestep),
        simple_loss_noise_seed=int(args.simple_loss_noise_seed),
    )
