...
For `das`, `traj_tracin`, `dtrak`, and `end_tracin`, attribution reads
`attribution_sample_seed` and `attribution_sample_index` from the target
algorithm config in `<dataset>/dataset_config.py`. These fields select a sample
and trajectory previously generated by `00_sample.sh`; attribution does not
generate a missing sample. Defaults may differ by dataset and algorithm, so
check the target config. `journey_trak` is an exception: it uses `QUERY` as
conditioning and constructs its query trajectory inside the attribution engine.
...
`00_sample_unprompted.sh` must run first, and
`01_data_attribution_unprompted.sh` only consumes the saved sample and
trajectory. `journey_trak` constructs its query trajectory inside attribution.
